...
cUsDF_myjq(document).ready(function($) {
    ...
    try{
        ...
        //LOADING UI BOX
        $( ".cUsDF_preloadbox" ).delay(1500).fadeOut();

        $('.btn').tooltip();

        //UI TABS
        //$( "#cUsDF_tabs" ).tabs({active: false});
        ...
        cUsDF_myjq('.sign-in').click(function() {
            cUsDF_myjq('.signup-form').slideToggle('slow');
            cUsDF_myjq('.login-form').slideToggle('slow');
        });
    ...
    try{
        ...
        var validateLogin = oLoginForm.validate({
            ...
            submitHandler: function(e) {
                ...
                //console.log(e);
                $('.cUsDF_LoginUser').html('Sign In...').attr('disabled', true);
                    ...
                    success: function(data) {
                        ...
                        switch(data.status){
                            ...
                            case 1:
                                ...
                                $('.cUsDF_LoginUser').html('Login').attr('disabled', false);

                                break;

                            //CURRENT USER DON'T HAVE DEFAULT DONATION FORM
                            case 2:

                                $('.cUsDF_LoginUser').html('Error . . .');

                                message = '<p>To continue, you will need to create a default Donation Form.</p>';
                                ...
                                //$.messageDialogLogin('Default Donation Form Required');
            ...
            rules: {
                ...
                email: {
                    ...
                    remote: {
                        ...
                        data: {
                             action: 'cUsDF_verifyCustomerEmail',
                             email: function() {
    ...
    try{
        ...
        cUsDF_myjq('.btn-skip').click(function(e) {

            e.preventDefault();
            var oThis = cUsDF_myjq(this);
            oThis.hide();
            ...
            //cUsDF_myjq(".img_loader").show();
            cUsDF_myjq('.loading').show();
            ...
            oSignUpData += '&action=cUsDF_createCustomer';
            //console.log(oSignUpData);

            //AJAX POST CALL cUsDF_createCustomer
            cUsDF_myjq.ajax({ type: "POST", dataType:'json', url: ajax_object.ajax_url, data: oSignUpData,
                success: function(data) {
                    ...
                    switch(data){
                        ...
                        case 1:
                            ...
                            cUsDF_myjq('.notice_success').html(message).show().delay(4900).fadeOut(800);
                            //cUsDF_myjq("#cUsFC_SendTemplates").colorbox.close();
                            setTimeout(function(){
                                cUsDF_myjq('.step3').slideUp().fadeOut();
                                cUsDF_myjq('.step4').slideDown().delay(800);
                                cUsDF_myjq('#cUsDF_SendTemplates').val('Create my account').attr('disabled', false);
                                location.reload();
                            },2000);
                            break;
                        //OLD USER - LOGING
                        ...
                        default:
                            ...
                            cUsDF_myjq('.notice_error').html(message).show();
                            cUsDF_myjq('#cUsDF_SendTemplates').val('Create my account').attr('disabled', false);
                            cUsDF_myjq("#cUsDF_SendTemplates").colorbox.close();
                            break;
                    }
        ...
        cUsDF_myjq('.notice_error').html('Unfortunately there has being an error during the application. ' + err).slideToggle().delay(9000).fadeOut(2000);
        cUsDF_myjq('#cUsDF_SendTemplates').val('Create my account').attr('disabled', false);
    }
