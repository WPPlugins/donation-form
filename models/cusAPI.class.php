 ...
 * @api
 * @version 0.4
 * @since 2.0 First time this was introduced into Donation Form plugin.
...
class cUsComAPI_DF {
     ...
     * @api
     */
    var $enviroment = cUsDF_API_ENV;
    var $wpApiAccountKey = cUsDF_API_ACC;
    var $wpApiKey = cUsDF_API_AKY;
    ...
    public function getAPICredentials($cUs_email, $cUs_pass) {
        ...
        if( $this->_isCurl() ){
            ...
            curl_setopt($ch, CURLOPT_HTTPHEADER, array('X-ContactUs-Request-URL: ' . $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI'], 'X-ContactUs-Signature: wpdf|3.0|' . $this->getIP()));
    ...
    public function createCustomer($postData, $credential) {
        ...
        if (!strlen($postData['fname'])) {
            ...
            if( $this->_isCurl() ){
                ...
                $strCURLOPT .= '&Promotion_Code=WPDF';
                $strCURLOPT .= '&Version=wpdf|3.0';
                ...
                curl_setopt($ch, CURLOPT_HTTPHEADER, array(
                    ...
                    'X-ContactUs-Signature: wpdf|3.0|' . $this->getIP()
                ));
    ...
    public function getDefaultDeepLink($cUsDF_API_getFormKeys) {

        if (empty($cUsDF_API_getFormKeys))
            return FALSE;

        //$cUsMC_API_getKeysResult = $this->getFormKeysData($cUs_API_Account, $cUs_API_Key);
        $cUs_jsonKeys = json_decode($cUsDF_API_getFormKeys);
    ...
    public function resetData() {
        ...
        delete_option('cUsDF_settings_default_deep_link_view');
        delete_option('cUsDF_settings_form_id');
        delete_option('cUsDF_settings_intro_hints');

        cUsDF_page_settings_cleaner(); //REMOVE CUSTOM SETTINGS BY PAGE
        cUsDF_inline_shortcode_cleaner(); //REMOVE SHORTCODES
