...
class cUsDF_contactus_form_Widget extends WP_Widget {

	function cUsDF_contactus_form_Widget() {
		$widget_ops = array( 
			'description' => __('Displays Donation Form by ContactUs.com', 'contactus_donation_form')
		);
		$this->WP_Widget('contactus_donation_form_Widget', __('Donation Form by ContactUs.com', 'contactus_donation_form'), $widget_ops);
	}
    ...
    function form($instance) {
        ...
        if( $instance) {
            ...
            $form_key = get_option('cUsDF_settings_form_key'); //get the saved form key
        }
        ?>
	...
	function widget( $args, $instance ) {
        ...
        cUsDF_contactus_donation_form(array_merge($args, $instance));
	}
}
/*
...
*/
function cUsDF_contactus_donation_form($args = array()) {
    extract($args);
    ...
    if(empty($cUs_form_key))
        $cUs_form_key = get_option('cUsDF_settings_form_key'); //get the saved form key
    
    if(strlen($cUs_form_key)){
        $xHTML  = '<div id="cUsDF_form_widget" style="clear:both;width:100%;min-height:230px;margin:10px auto;">';
        $xHTML .= '<script type="text/javascript" src="' . cUsDF_ENV_URL . $cUs_form_key . '/inline.js"></script>';
        $xHTML .= '</div>';
